...
## 2026-09-07 04:01 UTC: headless desplegado, parada conservada
...
[Protocolo y mediciones de rendimiento](HEADLESS-PERFORMANCE.md): la comparación extendida de seis minutos por versión cumple el criterio. Mediana RSS 415,03 → 229,91 MiB; pico incluyendo informes 435,40 → 317,82 MiB; CPU 124,59 → 91,03 segundos; p95 de lote 9.586,48 → 4.506,77 ms. Las medianas finales headless son 230,34 y 230,29 MiB. No se observa crecimiento sostenido en esos segmentos; el alcance es finito y el host compartido. Se conservan todos los ensayos cortos que incumplieron el criterio. El PNG del replay se inspeccionó visualmente: español, fondo oscuro, paleta y cuenta vacía legibles. Es una prueba sintética de formato, no una captura de la cuenta operativa.

**Estado de despliegue:** `botpoly.service` está habilitado y `active/running` desde las **03:59:54 UTC**, PID **2612462**, cero reinicios automáticos. Ejecuta Node 24.20.0 con `dist/src/app/run.js` mediante el lanzador compartido. La cuenta sigue en paper con `Límite de pérdida diaria`: 13 órdenes, 13 fills, 11 filas de posiciones, seis reservas y tres liquidaciones. No se reanudaron entradas. La comprobación a las **04:01:45 UTC** tiene conexión y evaluaciones actuales, con outbox vacía. [Evidencia de migración, proceso, Telegram e informes](evidence/headless-deployment-20260907.json).

Se ejecutó el instalador con autorización y autenticación del usuario. Detuvo el servicio anterior antes de reemplazar su unidad y creó una copia consistente como gabo, con permisos privados. La comprobación completa de integridad, hash y conservación tardó aproximadamente once minutos con el motor detenido. El manifiesto `.runtime/backups/headless-2026-09-07T03-48-27-674Z-552b337f.json` registra `integrity=ok`, `preserved=true`, cuenta idéntica y hashes idénticos de órdenes, fills, posiciones, reservas y liquidaciones antes/después. SHA-256 de la copia SQLite: `0659351903d1d0b305328ddcb9f4ce506bab3b7a1f835ed9766490b82d379880`. Solo se añadió el parámetro ausente `max_oper_per_hour=15`; se reconstruyeron once cupos históricos, ninguno pendiente ni dentro de la ventana activa en este corte. Se conservan los otros parámetros, pérdidas y parada.

La segunda ejecución real de `pnpm start` salió con **75** antes de crear otro motor. El proceso activo no tiene sockets de escucha TCP/UDP y conserva el endurecimiento de systemd. No había trabajador hijo tras generar el informe. Las primeras capturas nuevas contienen 200 observaciones Gamma y 187 precios medios por minuto; todavía no constituyen la cobertura requerida para la política experimental. El historial anterior no se convirtió en observaciones nuevas ficticias.

Telegram confirmó el aviso de despliegue y `getMyCommands` devolvió exactamente los trece comandos en minúsculas. El informe administrativo `reports/paper-headless-service-verification-20260907/`, capturado a las **04:00:29 UTC**, conserva PnL neto acumulado **−30,1154 US$**, realizado −24,6712 US$, abierto −5,4441 US$, comisiones 2,9590 US$ y gas modelado 0,0584 US$. Se verificaron los hashes de todos sus archivos y se inspeccionó el PNG de esta cuenta paper. El padre registró el resultado y Telegram aceptó texto, PNG, HTML, Markdown, JSON, CSV y manifiesto; también aceptó texto/PNG horarios T04. La solicitud administrativa utilizó la cola persistente existente, **sin simular un `/report` recibido de un usuario**. Aceptación de la API no acredita lectura humana; las valoraciones abiertas pueden cambiar.

No se ejecutó la suite heredada `test:integration` contra APIs públicas: no valida por sí sola los contratos actuales. No se ocultaron incompatibilidades externas mediante capturas silenciosas. El reinicio completo del servidor y las órdenes con fondos reales quedan fuera de esta validación; el servicio desplegado se comprobó durante minutos, no durante varios días.
